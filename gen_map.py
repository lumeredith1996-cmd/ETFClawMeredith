...
function switchChannel(ch){
  document.querySelectorAll('.chan-btn').forEach(function(b){b.classList.remove('active')});
...
function showPopup(code){
  var d=POPUP_DATA[code];if(!d)return;
  document.getElementById('popupBody').innerHTML=d;
...
function closeOnBg(e){if(e.target===document.getElementById('popupOverlay'))closePopup();}
...
html = (
    ...
    '<div class="channel-bar">\n  <div class="channel-label">选择投资渠道</div>\n  <div class="channel-tabs">\n    <button class="chan-btn active" id="chan-mainland" onclick="switchChannel(\'mainland\')">🇹🇨 内地投资者直接可买 <span style="opacity:0.65;font-size:11px;font-weight:400">(' + str(cnt_mainland) + ')</span></button>\n    <button class="chan-btn" id="chan-overseas" onclick="switchChannel(\'overseas\')">🌍 境外账户可买 <span style="opacity:0.65;font-size:11px;font-weight:400">(' + str(cnt_overseas) + ')</span></button>\n  </div>\n</div>\n'
    '<div id="tab-mainland" class="tab-content active">' + mainland_html + '</div>\n'
    '<div id="tab-overseas" class="tab-content">' + overseas_html + '</div>\n'
    '<div class="ch-intro"><strong>🌐 四大跨境投资渠道对比</strong> — 不同渠道在投资门槛、额度限制、可投标的范围等方面各有不同，适合不同需求的投资者。</div>\n'
    ...
    '</div>\n'
    '<div id="popupOverlay" onclick="closeOnBg(event)">\n'
    '  <div class="popup-box">\n'
    '    <div class="popup-header"><h3>📋 产品详情