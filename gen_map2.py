...
for code, info in HOLIDAYS_RAW.items():
    HOLIDAYS[code] = {'holidays': parse_holidays(info['raw']), 'note': info['note']}
...
all_mainland_codes = set(p['code'] for p in mainland_prods) | set(p['code'] for p in crossborder_prods)
cnt_mainland = len(all_mainland_codes)
...
.header{background:#1a1d2e;color:#fff;padding:18px 24px;display:flex;align-items:center;justify-content:space-between;flex-wrap:wrap;gap:12px}
.header h1{font-size:20px;font-weight:700;display:flex;align-items:center;gap:8px}
.header-right{font-size:12px;opacity:0.55}
.nav{display:flex;gap:8px;padding:12px 24px;background:#fff;border-bottom:1px solid #e2e5ef;flex-wrap:wrap}
.nav a{color:#6b7280;text-decoration:none;font-size:14px;padding:6px 14px;border-radius:6px;transition:all 0.15s;display:inline-block}
...
.prod-chip{background:#f0f2f8;border-radius:6px;padding:7px 12px;font-size:13px;cursor:pointer;display:inline-block;transition:all 0.15s;margin:3px;border:1px solid transparent}
.prod-chip:hover{background:#e2e5ef;color:#1a1d2e}
.ch-intro{padding:16px 24px;background:#fff;border-top:1px solid #e2e5ef;font-size:13px;color:#6b7280;line-height:1.6}
.ch-table{width:100%;border-collapse:collapse;background:#fff;border-radius:0 0 10px 10px;overflow:hidden;box-shadow:0 2px 8px rgba(0,0,0,0.06);border:1px solid #e2e5ef;border-top:none}
...
#popupOverlay{display:none;position:fixed;inset:0;z-index:1000;background:rgba(0,0,0,0.5);backdrop-filter:blur(4px);justify-content:center;align-items:center;padding:20px}
...
# Build HTML by concatenation to avoid quote conflicts
html_parts = []
...
html_parts.append('<div class="channel-bar">\n  <div class="channel-label">选择投资渠道</div>\n  <div class="channel-tabs">\n')
html_parts.append("    <button class=\"chan-btn active\" id=\"chan-mainland\" onclick=\"switchChannel('mainland')\">🇹🇨 内地投资者直接可买 <span style=\"opacity:0.65;font-size:11px;font-weight:400\">(" + str(cnt_mainland) + ")</span></button>\n")
html_parts.append("    <button class=\"chan-btn\" id=\"chan-overseas\" onclick=\"switchChannel('overseas')\">🌍 境外账户可买 <span style=\"opacity:0.65;font-size:11px;font-weight:400\">(" + str(cnt_overseas) + ")</span></button>\n")
html_parts.append('  </div>\n</div>\n')
html_parts.append('<div id="tab-mainland" class="tab-content active">' + mainland_html + '</div>\n')
html_parts.append('<div id="tab-overseas" class="tab-content">' + overseas_html + '</div>\n')
html_parts.append('<div class="ch-intro"><strong>🌐 四大跨境投资渠道对比</strong> — 不同渠道在投资门槛、额度限制、可投标的范围等方面各有不同，适合不同需求的投资者。</div>\n')
html_parts.append('<div style="padding:0 24px 24px">\n<table class="ch-table"><thead><tr><th style="width:150px">渠道</th><th style="width:80px">可参与地区</th><th style="width:180px">投资门槛</th><th style="width:150px">额度限制</th><th>资金投向</th></tr></thead><tbody>' + chan_rows + '</tbody></table>\n</div>\n')
html_parts.append('<div id="popupOverlay" onclick="closeOnBg(event)">\n')
html_parts.append('  <div class="popup-box">\n')
html_parts.append('    <div class="popup-header"><h3>📋 产品详情</h3><button class="popup-close" onclick="closePopup()">×</button></div>\n')
html_parts.append('    <div class="popup-body" id="popupBody"></div>\n')
html_parts.append('  </div>\n')
html_parts.append('</div>\n')
html_parts.append("<script>\nvar POPUP_DATA = " + popup_json + ";\n")
html_parts.append("function switchChannel(ch){")
html_parts.append("  document.querySelectorAll('.chan-btn').forEach(function(b){b.classList.remove('active')});")
html_parts.append("  document.getElementById('chan-'+ch).classList.add('active');")
html_parts.append("  document.querySelectorAll('.tab-content').forEach(function(t){